        ...
        case .movies:
            clientApi.getMovies { [weak self] (result) in
                ...
                case .succeeded(let movies):
                    let items: [ItemDetailViewModel] = movies.feed.results.map { ItemDetailViewModel(withClientApi: strongSelf.clientApi, feedItem: $0) }
        ...
        case .songs:
            clientApi.getSongs { [weak self] (result) in
                ...
                case .succeeded(let songs):
                    let items: [ItemDetailViewModel] = songs.feed.results.map { ItemDetailViewModel(withClientApi: strongSelf.clientApi, feedItem: $0) }
